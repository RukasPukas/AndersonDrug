$(function () {
  ...
  function updateAll(weightLbs) {
    ...
    diltiazemAdultSpan.textContent = `≈ (${diltiazemAdult.toFixed(2)}mg over 2-5 minutes)`

    //Diltiazem Geri Dose
    let diltiazemGeri = kg* 0.1;
    if (diltiazemGeri > 10) diltiazemGeri = 10;
    diltiazemGeriSpan.textContent = `≈ (${diltiazemGeri.toFixed(2)}mg over 2-5 minutes)`

    //Etomidate Dose
    let etomidate = kg*.1;
    if (etomidate >10) etomidate = 10;
    etomidateVol = etomidate / 2;
    etomidateSpan.textContent = `≈ (${etomidate.toFixed(2)}mg ${etomidateVol.toFixed(2)}ml )`
  }
